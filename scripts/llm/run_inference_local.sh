...
if [[ ! -f "config/llm.local.env" ]]; then
  echo "[run] Falta config/llm.local.env"
  echo "[run] Completa config/llm.local.env y vuelve a ejecutar."
  exit 1
fi
...
PYTHONPATH=src .venv/bin/python run_pipeline.py --only 3,4 --connections config/connections.yaml
